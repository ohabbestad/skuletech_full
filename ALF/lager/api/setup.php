...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    try {
        ...
        $stmt = lager_pdo()->prepare(
            'INSERT INTO lager_users (username, role, password_hash, active)
             VALUES (?, ?, ?, 1)
...
<head>
  ...
  <title>Lageroppsett</title>
  <script src="https://cdn.tailwindcss.com"></script>
</head>
<body class="bg-slate-100 min-h-screen flex items-center justify-center p-6">
  <main class="bg-white border border-slate-200 rounded-xl shadow-sm p-6 w-full max-w-lg">
    <h1 class="text-2xl font-bold text-slate-900 mb-2">Lageroppsett</h1>
    <p class="text-slate-600 mb-6">Bruk denne sida til å lage eller byte passord for tørrvarelageret.</p>
